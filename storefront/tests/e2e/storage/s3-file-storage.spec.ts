...
// Minimal valid JPEG (1x1 pixel) — same bytes used in review image-upload tests
...
/** Register a test customer and return auth token */
async function getAuthToken(): Promise<string> {
  const email = `e2e-s3-${Date.now()}@test.local`;
  ...
  if (!regRes.ok) throw new Error(`Auth register failed (${regRes.status})`);
  const { token: regToken } = (await regRes.json()) as { token: string };

  // Create customer record
  const custRes = await fetch(`${BACKEND_URL}/store/customers`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({ email, first_name: "S3", last_name: "Test" }),
  });
  if (!custRes.ok)
    throw new Error(`Create customer failed (${custRes.status})`);

  // Login for fresh token
